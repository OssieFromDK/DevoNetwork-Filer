...
AddEventHandler("cn-fotovogn:createdefault", function(list)
    for _, item in pairs(list) do
        ...
        local vehicle = CreateVehicle(GetHashKey(item.Model), item.coords[1], item.coords[2], item.coords[3], item.coords[4], true, false)
        TriggerServerEvent("cn-fotovogn:sendvogn",VehToNet(vehicle),{coords=item.data.coords,turned=item.data.turned,limit=item.data.limit,radius=item.data.radius})
    end
end)
...
function checkSpeed(limit)
    ...
    if kmhspeed > limit+4 and driver == pP then
        ...
        TriggerServerEvent('cn-fotovogn:betal',kmhspeed,limit,VehToNet(vehicle))
    end
    Wait(3000)
end
