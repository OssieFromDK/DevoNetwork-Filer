...
$(document).ready(function(){
  ...
  window.addEventListener('message', function(event){
    ...
    if(item.updateBalance == true) {
      ...
      $('.currentWallet').html(addCommas(event.data.wallet +' $'));
  ...
  document.onkeyup = function (data) {
    if (data.which == 27 ) {
      $.post('http://vrp_bank/close', JSON.stringify({}));
    }
  };
  // Handle Button Presses
  $(".btnWithdraw").click(function(){
      $.post('http://vrp_bank/withdraw', JSON.stringify({}));
  });
  $(".btnDeposit").click(function(){
      $.post('http://vrp_bank/deposit', JSON.stringify({}));
  });
  $(".btnTransfer").click(function(){
      $.post('http://vrp_bank/transfer', JSON.stringify({}));
  });
  $(".btnBalance").click(function(){
      $.post('http://vrp_bank/balance', JSON.stringify({}));
  });
  $('.btnQuick').click($.throttle( 2000, true, function(e){
    $.post('http://vrp_bank/quickCash', JSON.stringify({}));
  }));
  ...
  $("#withdraw-form").submit(function(e) {
      e.preventDefault();
      $.post('http://vrp_bank/withdrawSubmit', JSON.stringify({
  ...
  $("#deposit-form").submit(function(e) {
      e.preventDefault();
      $.post('http://vrp_bank/depositSubmit', JSON.stringify({
  ...
  $("#transfer-form").submit(function(e) {
      e.preventDefault();
      $.post('http://vrp_bank/transferSubmit', JSON.stringify({
