      ...
      menudata.onclose = function(player)
		...
		vRP.getUData({user_id,"vRP:head:overlay",function(value2)
          ...
          if vRP.tryFullPayment({user_id,price}) then
            if price > 0 then
              TriggerClientEvent("pNotify:SendNotification", player,{text = "Betalte: "..price.." $", type = "info", queue = "global", timeout = 5000, layout = "centerRight",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
            end
          else
