		...
		if not InBlacklistedVehicle then
			...
			if nearPump and IsCloseToLastVehicle then
				...
				elseif IsFueling then
					...
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Tryk ~g~E~w~ for at afrbyde optankningen af dit køretøj.")
					...
					DisableControlAction(0, 44, true) -- Intet cover
					DisableControlAction(0, 47, true) -- Intet  Krydsarme
					DisableControlAction(0, 74, true) -- Intet vink
					DisableControlAction(0, 246, true) -- Intet Thumbsup
				...
				elseif fuel > 95.0 then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Dit køretøj er allerede tanket op.")
				elseif cash <= 0 then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Du har ikke nok ~r~penge~w~ til at tanke.")
				else
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Tryk ~g~E~w~ for at tanke. ~r~3.00 $~w~/L.")
			...
			elseif NearVehicleWithJerryCan and not nearPump and Config.EnableJerryCans then
				...
				if IsFuelingWithJerryCan then
					DrawText3Ds(coords.x, coords.y, coords.z + 0.5, "Tryk ~g~E~w~ for at stoppe optankningen af køretøjet. Benzin: " .. fuel .. "%")
					...
					DisableControlAction(0, 289, true) -- F2
					DisableControlAction(1, 323, true) -- Handsup (X)
					DisableControlAction(1, 47, true) -- Kryds Hænder
					DisableControlAction(1, 74, true) -- Vink
					DisableControlAction(1, 246, true) -- Thumbsup
					...
					DrawText3Ds(coords.x, coords.y, coords.z + 0.5, "Tryk ~g~E~w~ for at bruge din ~r~benzin dunk~w~ til at tanke køretøjet op.")
